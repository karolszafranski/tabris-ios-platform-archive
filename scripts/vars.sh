...
# Usage: `exit_if_command_fail ./path/to/your/script.sh`, it will exit if exit
# stause of the provided command/script is not equal to 0
function exit_if_command_fail {
	$@
	EXIT_CODE=$?
	if [[ "$EXIT_CODE" -ne 0 ]]; then
		print_info "Failed to execute: \"$@\" with exit code: $EXIT_CODE. Aborting..."
		exit $EXIT_CODE
	fi
}
